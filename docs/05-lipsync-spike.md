...
原来 `DEFAULT_FACE_ZOOM` 是 `flashhead_avatar.py` 的模块常量，改一次要动代码、重启
服务，而且是全局的——一个角色是半身、另一个是特写，只能迁就其中一个。中间那段时间
还出现过「代码写 1.2、实际跑 1.6」，界面上没有任何提示。

现在角色编辑器里有一根滑杆（`avatar.face_zoom`，1.0–2.3），一路传到
`Avatar(face_zoom=)`。字段缺省时服务仍然用模块常量，所以老配置行为不变。

**改完要重新点「准备形象」**——裁切是烘进缓存的。`_cache_matches_source()` 会比对
缓存里记的 `face_zoom`，不一致就自动重建，所以不用手动删缓存，但要真的去点一次。
滑杆下面的说明写了这句。

数值从面板过一道校验（1.0–2.5，非数字拒绝），因为它是从浏览器来的：服务端不挑，
一个离谱的值会裁出几个像素再放大到满屏，报错的地方离原因很远。

#### 那「贴回原图」呢：算出来只值 +6%，先不做

一直挂在上面那条"真要往上走"的路线里，现在把账算清楚了，结论是**在别的东西不动的
前提下不值得做**。

关键是 `STREAM_MAX_HEIGHT = 576` 这个上限。生成帧是 512×512，低于上限，原样发出；
而贴回去之后画面变成参考图那么大（1024），发出前要缩到 576，缩放系数 0.5625——**刚好
把裁紧赚来的那点全吐回去**。用小玲那套素材实算（`_aligned_reference` 真跑，不是估）：

| 方案 | 送到眼前的人脸高度 |
| --- | --- |
| 现在：zoom 1.2，直接发 512 | 219 px |
| zoom 1.8，直接发 512 | **329 px（+50%）** |
| 贴回 1024 再缩到 576 | 233 px（+6%），**任何 zoom 都是这个数** |

最后一行是重点：贴回去之后 zoom 调多少都一样，因为决定结果的是"人脸在整张图里占
多大"，而那是参考图本身决定的，与裁多紧无关。

所以 **zoom 1.8 直接发才是眼下最划算的**：+50%，而且 crop_side 645 > 512，仍然是
缩小，细节是真的不是插值。上限约 2.3（crop_side 掉到 504，开始放大）。

贴回去要变得有意义，必须**同时**把 `STREAM_MAX_HEIGHT` 提到 1024 左右，那时是
414 px、比现在多 89%，而且身体还在画面里。代价是每帧 JPEG 面积变四倍。这条路留着，
但它是"提上限 + 贴回去"一整件事，不能只做一半——上面那张表就是只做一半的样子。

#### 真跑了一次，"脖子露馅"这条基本不成立

前面（以及更早那节）一直把"贴到哪个身体上"当成最大障碍：模型会生成头部运动，头在
动而贴回去的身体不动，脖子处会露馅。**真生成一段量过之后，这条基本不成立。**

同一段 4.22 秒语音，zoom 1.8 生成 96 帧，贴回参考图，逐帧算相邻帧差：
...
看起来是个悬崖，但**沿着下边界逐行看，运动早在到达边界之前就衰减掉了**：

```
y=725  0.31      y=770  0.06
y=740  0.24      y=785  0.00  <- 边界
y=755  0.20      y=800  0.00
```

运动热力图（`seam-motion-map.png`）说明了原因：**动的只有眼睛、嘴、几缕头发和项链
坠子**，肩膀和胸口在生成帧里本来就几乎不动。所以边界穿过的是一片本来就静止的区域，
"动 / 不动"的落差在那里接近零。

#### 但发现了另一个坑，而且是结构性的

第一版实验把生成帧贴回**整张参考图**，这是错的。参考图要裁剪，正是为了让生成画面
和待机视频取景一致（见前面「取景对齐」）——直接显示整张图，等于把那件事撤销了，
她一开口画面就会拉宽。

**贴回的目标必须是"当前 zoom 会用的那个裁剪框"，不是整张图。** 也就是：按 1.2 裁
出 967×967 当画布，把 1.8 生成的 645×645 贴在里面 (211, 140) 的位置。这样取景和
今天完全一致，只是头部换成了更细的像素。实验里的 `d-zoom18-pasted-aligned.mp4`
就是这么做的，`c-zoom18-pasted.mp4` 是错的那版，留着做对照。

所以 `_aligned_reference` 现在多返回 `crop_x` / `crop_y`——没有裁剪框的原点，就无法
把生成帧放回它原本的位置。

#### 最后用眼睛判的：不做了

贴回去可行、脖子那关也过了，但把 `a-zoom12-direct` 和 `d-zoom18-pasted-aligned`
并排放出来看，**肉眼看不出差别**。再按 1:1 裁出嘴部（两边都不缩放，缩放正是会把
判断对象污染掉的操作）：右边 509×254，左边 270×134，像素多 1.9 倍，**但唇纹和齿缘
没有跟着多出来**。

原因大概是模型在 512 上生成的内容本身就是柔的——裁得更紧只是把同样柔的东西铺开，
并没有多出真实细节。前面那张"zoom 2.0 牙齿逐颗分明"的表和这条不矛盾：那是把嘴部
裁剪放大到同一尺寸对比，1:1 看确实有差；但在正常观看尺寸下，这点差异被冲掉了。

所以**"提上限 + 贴回去"不做**。它要付出的是每帧 JPEG 面积约 3.5 倍、一套合成代码、
以及 `crop_x`/`crop_y` 之外更多的状态，换来的是一个看不出来的差别。

对滑杆的意义也跟着变了：**它的价值是控制取景，不是提清晰度**。想要半身就往 1.0
调，想要特写就往上调；别指望调高了嘴会明显变清楚。

这是这份文档里第六次「数字说有提升、图片说没有」。
...
### 贴回原始画面：这条成立，而且解决的是取景不是清晰度

前面那节把"贴回去"理解成"贴回参考图、显示整张图"，算下来只值 +6%，否掉了。**理解错
了。** 真正的做法是：方块贴回它被抠走的那个位置，**输出保持原始尺寸和构图**——720×1280
抠出 720×720 生成，贴回去还是 720×1280。

这样一来脸的大小和位置按定义就和待机视频一致，**跳变不是"减轻"，是结构上不可能发生**。

灵感（720×1280，参考图是它自己的第 16 帧，裁剪框 720×720 落在 (0, 184)，即画面中部）
实测，取开口前后各一帧比对：
...
从循环中途切也只有 7 px，因为模型只动眼睛和嘴，其余姿态跟参考帧一致。

#### 顺带纠正一条我写错的提示

先前把 `framing_error_pct` 露出来时，提示写的是"开口时脸会大 75%，像镜头推近"。查
CSS 之后发现竖屏不是这样：待机视频和生成画布共用同一条
`object-fit: contain`，方形画面放进竖屏画框会**加上下黑边并把脸整体下移**（实测约
100 px），而不是推近。那个百分比描述的是"脸占生成帧的比例"，只有在待机视频也是方
的时候（小满、小玲的 +20%）才等于观感上的推近。提示文案已按这个区分改写。

#### 但看片子发现接缝处胸口对不齐，量完之后这条也否掉了

跳变确实没了，可是**衔接处胸口有几帧对不上**。第一反应是我这轮忘了羽化（方形那轮用了
遮罩，这轮直接硬贴）。补上羽化，行间跳变从 3.65 降到 0.63，硬边没了——**问题还在**。

量下去才看清是两件事，羽化一件都解决不了：

**一、生成方块整体就不是原图。** 生成内容与原图的差值沿边缘到中心都在 9.5～13.9，
**不随距离衰减**。模型把整个方块重画了一遍，不是"边没对齐"。缝放在哪里都会有约 10
个色阶的落差。

**二、缝下面冻结，缝上面在动。** 生成内容的帧间运动：上部 64 行 1.76，中部 1.52，
**下部 64 行（胸口）1.26**。羽化能糊掉一条线，糊不掉"上面在动、下面不动"。

#### 一条被推翻的旧结论

前面写过"边界处帧间变化已衰减到 0.06，所以身体不动看不出来"。那是在**小玲**身上量
的：方形素材，裁剪框 967 几乎覆盖整张 1024 图，边界落在画面最外圈的背景上。我把它
当成了普遍规律。

灵感是竖屏，裁剪框只有上下两条边，**下边正好横穿胸口**，同一个量是 1.26 而不是
0.06——差二十倍。**边界落在哪里，取决于素材，不取决于方法。**

#### 那竖屏还剩什么办法

唯一干净的是**把待机视频也裁成同一个正方形来显示**：没有黑边、没有接缝、没有推近，
全程一致。代价是待机时也只看到中间那块正方形，竖屏构图的上下会被切掉。

换一段不是竖屏的待机视频同样能解决——方配方时只剩 +20% 的推近，没有接缝问题。

#### 代价（若将来重做）：竖屏必须同时提 STREAM_MAX_HEIGHT

贴回去之后整帧是 720×1280，过 576 上限要缩到 324×576，脸从合成里的 323 px 掉到
146 px——**比今天只发方块的 230 px 还差**。而待机视频是浏览器直接播本地文件、不过这
条上限，所以会出现"待机清楚、一开口发糊"。

这和之前否掉的那次不同：那次是想额外买清晰度，验下来看不出、不值；这次是**别把已经
生成出来的东西在传输时扔掉三分之一**。竖屏要做就得把上限提到 1280 附近。
